...
export function mapDbError(err) {
  ...
  if (code === 'ER_ACCESS_DENIED_ERROR' || code === 'ER_DBACCESS_DENIED_ERROR') {
    ...
    const hint = emptyPw
      ? 'MYSQL_PASSWORD is empty in server/.env. If your MySQL user has a password, set MYSQL_PASSWORD to that exact value, save, and restart the API. If the password contains # or =, wrap it in double quotes, e.g. MYSQL_PASSWORD="your#pass".'
      : 'Verify MYSQL_USER and MYSQL_PASSWORD match a MySQL account that can log in (try mysql -u USER -p). On Windows, try DB_HOST=127.0.0.1. Run: node server/db/test-connection.js';
    const detail = err.sqlMessage ? ` (${err.sqlMessage})` : '';
    return { status: 503, message: `Database access denied. ${hint}${detail}` };
  }
  if (code === 'ER_BAD_DB_ERROR') {
    return { status: 503, message: 'Database does not exist. Run database/schema.sql and set MYSQL_DATABASE in server/.env.' };
  }
